...
if (isset($_POST['id'])) {
	$del = "delete from Ep where id = " .$_POST['id'];
	$res2 = mysqli_query($connection, $del);
	$added = "EP Eliminado";
}

$query = "select * from Ep";
...
<html>
	<head>
		<title>Viva Mexico| Delete EP</title>
					...
					<div id="form">
						...
						<?php
							while($row = mysqli_fetch_assoc($result)) {
								...
								echo '<p>EP mail: '.$row['email_ep'].'</p>';
								echo '<p>Manager mail: '.$row['email_man'].'</p>';
								echo '<p>Programa/Issue: '.$row['programa'].'</p>';
								echo '<input type="hidden" name="id" value="'.$row['id'].'">';
								echo '<input type="submit" value="Eliminar EP">';
								echo '</form>';
								echo '</div>';
							}
						?>
