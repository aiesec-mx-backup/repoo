...
if ($_POST) {
	$query = "update curso
			  set titulo = '".$_POST['titulo']."', descripcion = '".$_POST['desc']."',video_link = '".$_POST['video']."',pres_link = '".$_POST['pres']."',audio_link = '".$_POST['audio']."'
			  where id = " .$_GET['id'];

	$result = mysqli_query($connection, $query);

	if ($result)
		$added = "Curso modificado";
	else
		$added = "Error";

}

$in_query = "select * from curso where id=".$_GET['id'];
$in_res = mysqli_query($connection, $in_query);
$info = mysqli_fetch_assoc($in_res);
...
<div role="main" id="main" style="position: relative; right: -30px; top: 0px;">
	...
	<div class="wrapper">
		...
		<form method="post" action="">
			<span>Titulo</span><br />
			<span><input type="text" name="titulo" <?php echo 'value="'.$info['titulo'].'"' ?>></input></span><br />
			<span>Descripcion</span><br />
			<span><textarea name="desc" rows="4" cols="50"><?php echo $info['descripcion'] ?></textarea></span><br />
			...
			<span><input type="text" name="video" <?php echo 'value="'.$info['video_link'].'"' ?>></input></span><br />
			...
			<span><input type="text" name="pres" <?php echo 'value="'.$info['pres_link'].'"' ?>></input></span><br />
			<span>Audio</span><br />
			<span>Link completo al audio</span><br />
			<span><input type="text" name="audio" <?php echo 'value="'.$info['audio_link'].'"' ?>></input></span><br />
